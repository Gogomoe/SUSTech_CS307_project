_12307.api = {
    ...
    search_train_transship_by_keyword: function(request_form, cb_ok=this.default_cb_ok, cb_err=this.default_cb_err ){
        var request_url = `${this.url}train/transship/from/${request_form["from"]}/to/${request_form["to"]}/date/${request_form["date"]}`;
        dhx.ajax.get(request_url).then(cb_ok).catch(cb_err);
    },
